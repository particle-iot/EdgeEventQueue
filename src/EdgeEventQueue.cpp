...
int EdgeEventQueue::setup() {
    int res = diskQueue.start(path, sizeLimit, policy);

    return res;
}

void EdgeEventQueue::loop() {
...
int EdgeEventQueue::publish(const char *eventName, const char *eventData) {
...
// [static] 
int EdgeEventQueue::cloudServicePublish(const char *eventName, const char *eventData, PublishFlags publishFlags, size_t priority, std::function<int(CloudServiceStatus)> cb) {
    int res = 0;
